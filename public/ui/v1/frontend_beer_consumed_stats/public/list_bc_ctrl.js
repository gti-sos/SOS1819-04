    ...
    .controller("ListBeerConsumedStatsCtrl", ["$scope", "$http", function($scope, $http) {
        ...
        function refresh(API) {
            ...
            $http.get(API + "?limit=" + limit + "&offset=" + offset).then(function(response) {
                ...
                if ($scope.status == 404) {
                    $scope.status = $scope.status + " - Intentalo de nuevo";
                }
                $scope.data = "";
            });
        ...
        $scope.postData = function() {
            ...
            $http.post(API, {
                country: $scope.newBeer.country,
                year: parseInt($scope.newBeer.year),
                rating: parseFloat($scope.newBeer.rating),
                variation: parseFloat($scope.newBeer.variation),
                countryConsumition: parseInt($scope.newBeer.countryConsumition)
